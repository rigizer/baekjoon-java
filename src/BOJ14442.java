...
public class BOJ14442 {
    ...
    private static int bfs(char map[][], boolean[][][] visited, int n, int m, int k) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int d = 0; d < 4; d++) {
                ...
                // visited에 대한 조건을 깊이별로 주어야 하기 때문에 아래의 if~else if문에 대해 조건삽입
                if (map[y][x] == '1' && node.k > 0 && visited[y][x][node.k - 1] == false) { // 벽을 뚫을 수 있는 조건 추가
                    queue.offer(new Node(y, x, node.d + 1, node.k - 1));
                    visited[y][x][node.k - 1] = true;
                } else if (map[y][x] == '0' && visited[y][x][node.k] == false) {            // 일반 경로 탐색
    ...
    public static void main(String[] args) throws Exception {
        ...
        boolean visited[][][] = new boolean[n][m][k + 1];   // 벽을 뚫을 수 있는 횟수에 대한 visited를 3차원 배열을 통해 경우의 수를 나눔
        for (int i = 0; i < n; i++) {
            map[i] = br.readLine().toCharArray();
        }
        
        int result = bfs(map, visited, n, m, k);
        bw.write(result + "\n");
        bw.flush();
    }
}
